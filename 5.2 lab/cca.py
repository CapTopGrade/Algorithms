...
def train(X, Y, K = K, accuracy_coef = 1):
    ...
    def calculate_covariate_matrices(centrated_matrix_x, centrated_matrix_y, i):
        ...
        C_xx_r = centrated_matrix_x[i] @ centrated_matrix_x_T
        C_yy_r = centrated_matrix_y[i] @ centrated_matrix_y_T
        C_xy_r = centrated_matrix_x[i] @ centrated_matrix_y_T
        C_yx_r = C_xy_r.T

        C_xx_c = centrated_matrix_x_T @ centrated_matrix_x[i]
        C_yy_c = centrated_matrix_y_T @ centrated_matrix_y[i]
        C_xy_c = centrated_matrix_x_T @ centrated_matrix_y[i]
        C_yx_c = C_xy_c.T 

        return C_xx_r, C_yy_r, C_xy_r, C_yx_r, C_xx_c, C_yy_c, C_xy_c, C_yx_c
    ...
    def covariate_count_eig_weight_parallel(centrated_matrix_x, centrated_matrix_y, K=2):
        ...
        start = time.time()
        sum_C_xx_r = sum(result[0] for result in results)
        sum_C_yy_r = sum(result[1] for result in results)
        sum_C_xy_r = sum(result[2] for result in results)
        sum_C_yx_r = sum(result[3] for result in results)
        sum_C_xx_c = sum(result[4] for result in results)
        sum_C_yy_c = sum(result[5] for result in results)
        sum_C_xy_c = sum(result[6] for result in results)
        sum_C_yx_c = sum(result[7] for result in results)

        S_1_r = np.linalg.pinv(sum_C_xx_r) @ sum_C_xy_r @ np.linalg.pinv(sum_C_yy_r) @ sum_C_yx_r
        S_2_r = np.linalg.pinv(sum_C_yy_r) @ sum_C_yx_r @ np.linalg.pinv(sum_C_xx_r) @ sum_C_xy_r

        S_1_c = np.linalg.pinv(sum_C_xx_c) @ sum_C_xy_c @ np.linalg.pinv(sum_C_yy_c) @ sum_C_yx_c
        S_2_c = np.linalg.pinv(sum_C_yy_c) @ sum_C_yx_c @ np.linalg.pinv(sum_C_xx_c) @ sum_C_xy_c
        ...
        plt.title('Correlation Plot 2DCCA Parallel')
        plt.show()
        ...
        plt.title('Delta Values Frequency 2DCCA Parallel')
        plt.show()
    ...
    def covariate_count_eig_weight_cascade(centrated_matrix_x, centrated_matrix_y, K = 2):
        ...
        for i in range(0,K):
            ...
            C_xx_r = (1/z)*(centrated_matrix_x[i] @ centrated_matrix_x_T)
            C_yy_r = (1/z)*(centrated_matrix_y[i] @ centrated_matrix_y_T)
            ...
            if i == 0:
                sum_C_xx_r = np.zeros_like(C_xx_r)
                sum_C_yy_r = np.zeros_like(C_yy_r)
                sum_C_xy_r = np.zeros_like(C_xy_r)
                sum_C_yx_r = np.zeros_like(C_yx_r)
                sum_C_xx_c = np.zeros_like(C_xx_c)
                sum_C_yy_c = np.zeros_like(C_yy_c)
        ...
        plt.xlabel('Index')
        plt.ylabel('Correlation')
        plt.title('Correlation Plot 2DCCA Cascade')
        plt.show()
        ...
        plt.xlabel('Value')
        plt.ylabel('Frequency')
        plt.title('Delta Values Frequency 2DCCA Cascade')
        plt.show()
    ...
    print("Time spend parallel cca:")
    print(time_spend_parallel)
    print("Time spend cascade cca:")
...
#    }
#    
#    with open("data_CCA.json", "w") as json_file:
...
#    }
#  
#    with open("data_CCA_re.json", "w") as json_file:
#        json.dump(data_re, json_file)
        
    
    print("Train is finished. All data is saved to data_cca.json and data_cca_re.json")    
...
def CCA_2D(K = 2, X = None, Y = None, G = None, L = None, accuracy_coef = 2):
    ...
    try:
        with open("data_CCA.json", "r") as json_file:
        ...
        print("Max distance")
        print(thereshould)
        with open("data_CCA_re.json", "r") as json_file:
            data = json.load(json_file)
...
if __name__ == "__main__":
    res1, res2, reconstructed_x, reconstructed_y = CCA_2D()
    print("RES1:")
    print(res1)   
    print("RES2:")
    print(res2)   
